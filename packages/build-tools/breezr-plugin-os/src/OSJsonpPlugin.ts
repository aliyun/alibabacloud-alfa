...
export class OSJsonpWebpackPlugin {
  ...
  private _wrapCodeWithOSJsonp(id: string) {
    const injectVars = this.option.lite ? ['history', 'location'] : ['window', 'location', 'history', 'document', 'self', ...(this.option.injectVars || [])];
...
${jsonpCall}(${JSON.stringify(id)}, function(require, module, exports, context){ ${injectVars.map((item) => `var ${item} = context.${item === 'self' ? 'window' : item}`).join(';')};with(window.__CONSOLE_OS_GLOBAL_VARS_) { \n
